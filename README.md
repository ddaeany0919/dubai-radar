...
### ✅ 단기 목표 (Priority)
- [ ] **실제 국세청 API 연동**: 현재 시뮬레이션인 사업자 인증을 실제 `api.odcloud.kr` API와 연동하여 유효성 검사 구현.
- [ ] **실제 소셜 로그인 연동**: Supabase Auth를 사용하여 카카오/네이버 실제 로그인 붙이기.
- [ ] **알림 시스템 구현**: FCM(Firebase Cloud Messaging) 등을 연동하여 실제 "재고 입고" 푸시 알림 발송.

### 🚀 장기 목표 (Backlog)
- [ ] **UI/UX 고도화**: 현재 인라인 스타일로 되어 있는 CSS를 Tailwind CSS 클래스로 깔끔하게 리팩토링.
- [ ] **지도 최적화**: 마커가 많아질 경우를 대비한 클러스터링(Clustering) 적용.
- [ ] **관리자 페이지**: 전체 가게 및 유저 현황을 볼 수 있는 어드민 대시보드.
